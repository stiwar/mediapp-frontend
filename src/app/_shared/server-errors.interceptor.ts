            ...
            pipe(tap(event => {
                if (event instanceof HttpResponse) {
                    if (event.body && event.body.error === true && event.body.errorMessage) {
                        throw new Error(event.body.errorMessage);//si tiene algún bloque de error la petición http, lanza la excepción y entra al bloque catch (línea 26)
                    }/*else{
                ...
                return EMPTY; //es un observable vacío
            }));
    }
}